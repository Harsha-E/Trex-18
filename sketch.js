...
function setup() {
  ...
  gameover = createSprite(windowWidth/ 2  , windowHeight/ 3  , 70 , 100);
  gameover.scale = 0.5;
  gameover.addImage("gameover", gameoverimg); 
  gameover.visible = false ; 
  
  replay = createSprite(windowWidth/ 2  , windowHeight/ 2 , 30, 30);
  replay.scale = 0.5;
...
if( (keyDown("space") || touches.length > 0) && trex.y >= windowHeight - 60) {
        trex.velocityY = -13;
        jumpS.play();
        touches = [];
...
function spawnObstacles(){
 if (frameCount % 60 === 0){
   var obstacle = createSprite(windowWidth,windowHeight - 45 ,10,40);
...
function spawnClouds() {
 ...
 cloud.y = Math.round(random(windowHeight/4 ,windowHeight/2 ));
